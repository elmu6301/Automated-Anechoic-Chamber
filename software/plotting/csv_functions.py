...
def getSparamData3D(csv_filename,sparam,frequency):
    ...
    freq_data = np.unique(freq_raw)
    closest_freq = util.findClosest(freq_data, len(freq_data), frequency)

    # print(f"closest_freq = {closest_freq * 1e-10}")
    # pdb.set_trace()
    assert closest_freq in freq_data
    ...
    # assert len(theta)*len(phi)*len(freq_data) == len(param_raw)
...
def getThetaCutData(csv_filename,sparam,frequency,theta):
    ...
    freq_unique = np.unique(freq_raw)
    closest_freq = util.findClosest(freq_unique, len(freq_unique), frequency)
    assert closest_freq in freq_unique

    theta_unique = np.unique(theta_raw)
    closest_theta = util.findClosest(theta_unique, len(freq_unique), theta)
    assert closest_theta in theta_unique
