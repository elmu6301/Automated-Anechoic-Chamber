...
def run_Align():
    ...
    printf('\t\t%f std. devs. above the ambient level.'%((aligned_value-np.mean(ambient_values))/np.std(ambient_values)))
    ...
    for idx__test_theta in np.arange(test_theta_steps):
        for idx__probe_phi in np.arange(probe_phi_steps):
            for idx__test_phi in np.arange(test_phi_steps):
                ...
                measurement_number += 1
                test_theta_orientation = test_theta_start+idx__test_theta*(((test_theta_end-test_theta_start)/(test_theta_steps-1)) if test_theta_steps!=1 else 0)
                test_phi_orientation   = test_phi_start+idx__test_phi*(((test_phi_end-test_phi_start)/(test_phi_steps-1)) if test_phi_steps!=1 else 0)
                probe_phi_orientation  = probe_phi_start+idx__probe_phi*(((probe_phi_end-probe_phi_start)/(probe_phi_steps-1)) if probe_phi_steps!=1 else 0)
