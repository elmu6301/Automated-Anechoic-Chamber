...
class VNA_HP8719A:
    ...
    def connect_VNA(self):
        ...
        # Download https://www.keysight.com/main/software.jspx?id=2175637&pageMode=CV&cc=US&lc=eng
        # print("All connected GPIB devices: ")
        # print(rm.list_resources()) #this prints all connected devices (not needed in code but should run once
        # to get device name)
        # print("")
        try:
            ...
            return False
        # instrument.read_termination = '\n'      #Set the read termination character  (due to VNA's data output,
        # this truncated the output data too early)
        ...
        if "HEWLETT PACKARD,8719A" in identify:     # If the instrument is correctly found
            ...
            print("Error: An instrument other than the VNA HP8719A has been connected. "
            ...
            # Is the Identify Error too specific? --> Note in User Manual where it's located in case
    ...
    def sparam_data(self, type_freq_sweep):
        ...
        self.instrument.write("NOOP")  # No operation + sets operation bit to complete (puts VNA back in listen mode
        # so can use front panel)

        # Set the frequency by using start/stop freq and default number of points (for demo) -->
        # THIS DOES WEIRD THINGS, REPLACE WITH GPIB COMMAND
        freq = []
        i = 1.01
        while i <= 3.01:
            freq.append(i)
        ...
        return np.array([freq, s11_db_only, s11_degree_only, s12_db_only, s12_degree_only, s21_db_only,
                         s21_degree_only, s22_db_only, s22_degree_only], dtype=object)

    # Remove column of 0's from VNA's output data by reformatting and splitting the input data string to create a
    # list with all 0's at odd list indices
    def data_formatting(self, data):
        data_col1 = data.replace(" ", "").replace("\n", ",").split(',')[0::2]  # [0::2] takes every other list
    ...
    def file_save(self, filename, data):  # Do we want to pass in user comments too?
        ...
        col_names = "Freq (GHz), S11 Mag (dB), S11 Phase (deg), S12 Mag (dB), S12 Phase (deg)," \
                    " S21 Mag (dB), S21 Phase (deg), S22 Mag (dB), S22 Phase (deg)"  # Will frequency ever not be GHz?
        np.savetxt(filename, data_transpose, delimiter=',', header=col_names, comments="")  # More specification
        # options available
    ...
    def plot(self, filename):
        ...
        # plt.show() #Shows the plots on screen but code halts until they are closed
        # (which is why I saved them as files instead)
